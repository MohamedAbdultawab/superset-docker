...
if [ "$SUPERSET_ENV" == "local" ]; then
    ...
    celery flower --app=superset.tasks.celery_app:app --address=0.0.0.0 --port=5555 &
    echo Started Celery workers[worker1, worker2] and Flower UI.

    # Start the prod web server
    gunicorn -w 10 -k gevent --timeout 120 -b  0.0.0.0:8088 --access-logfile - --error-logfile - --limit-request-line 0 --limit-request-field_size 0 "superset.app:create_app()"
else
    help
    exit -1
fi